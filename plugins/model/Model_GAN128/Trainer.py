...
class Trainer():
    ...
    def __init__(self, model, fn_A, fn_B, batch_size, perceptual_loss):
        ...
        generator = GANTrainingDataGenerator(
            self.random_transform_args, 220, 6, 2)
    ...
    def setup(self):
        distorted_A, fake_A, fake_sz64_A, mask_A, self.path_A, self.path_mask_A, self.path_abgr_A, self.path_bgr_A = self.cycle_variables(
            self.model.netGA)
        distorted_B, fake_B, fake_sz64_B, mask_B, self.path_B, self.path_mask_B, self.path_abgr_B, self.path_bgr_B = self.cycle_variables(
            self.model.netGB)
        real_A = Input(shape=self.model.img_shape)
        real_B = Input(shape=self.model.img_shape)

        if self.use_lsgan:
            self.loss_fn = lambda output, target: K.mean(
                K.abs(K.square(output-target)))
        else:
            self.loss_fn = lambda output, target: - \
                K.mean(K.log(output+1e-12)*target +
                       K.log(1-output+1e-12)*(1-target))
        ...
        if self.use_perceptual_loss:
            ...
            vggface_feat = Model(
                vggface.input, [out_size55, out_size28, out_size7])
            vggface_feat.trainable = False
        else:
            vggface_feat = None

        loss_DA, loss_GA = self.define_loss(
            self.model.netDA, real_A, fake_A, fake_sz64_A, distorted_A, vggface_feat)
        loss_DB, loss_GB = self.define_loss(
            self.model.netDB, real_B, fake_B, fake_sz64_B, distorted_B, vggface_feat)
        ...
        # Adam(..).get_updates(...)
        training_updates = Adam(lr=self.lrD, beta_1=0.5).get_updates(
            weightsDA, [], loss_DA)
        self.netDA_train = K.function([distorted_A, real_A], [
                                      loss_DA], training_updates)
        training_updates = Adam(lr=self.lrG, beta_1=0.5).get_updates(
            weightsGA, [], loss_GA)
        self.netGA_train = K.function([distorted_A, real_A], [
                                      loss_GA], training_updates)

        training_updates = Adam(lr=self.lrD, beta_1=0.5).get_updates(
            weightsDB, [], loss_DB)
        self.netDB_train = K.function([distorted_B, real_B], [
                                      loss_DB], training_updates)
        training_updates = Adam(lr=self.lrG, beta_1=0.5).get_updates(
            weightsGB, [], loss_GB)
        self.netGB_train = K.function([distorted_B, real_B], [
                                      loss_GB], training_updates)
    ...
    def define_loss(self, netD, real, fake_argb, fake_sz64, distorted, vggface_feat=None):
        ...
        if self.use_mixup:
            ...
            lam = dist.sample()
            # ==========
            mixup = lam * concatenate([real, distorted]) + \
                (1 - lam) * concatenate([fake, distorted])
            # ==========
            output_mixup = netD(mixup)
            loss_D = self.loss_fn(output_mixup, lam *
                                  K.ones_like(output_mixup))
            # output_fake = netD(concatenate([fake, distorted])) # dummy
            loss_G = 1 * self.loss_fn(output_mixup,
                                      (1 - lam) * K.ones_like(output_mixup))
        else:
            output_real = netD(concatenate(
                [real, distorted]))  # positive sample
            output_fake = netD(concatenate(
                [fake, distorted]))  # negative sample
        ...
        loss_G += K.mean(K.abs(fake_sz64 -
                               tf.image.resize_images(real, [64, 64])))
        # ==========
    ...
    def show_sample(self, display_fn):
        ...
        display_fn(self.showG_mask(
            tA, tB, self.path_mask_A, self.path_mask_B), "mask")
        # Reset the averages
