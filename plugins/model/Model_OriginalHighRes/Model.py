...
class Model():
    ...
    # good to let ppl know what these are...
    IMAGE_SHAPE = IMAGE_WIDTH, IMAGE_WIDTH, len('BRG')
    ...
    def initModel(self):
        ...
        self.autoencoder_A.compile(
            optimizer=optimizer, loss='mean_absolute_error')
        self.autoencoder_B.compile(
            optimizer=optimizer, loss='mean_absolute_error')
    ...
    def load(self, swapped):
        ...
        face_A, face_B = (hdf['decoder_AH5'], hdf['decoder_BH5']) if not swapped else (
            hdf['decoder_BH5'], hdf['decoder_AH5'])

        state_dir = os.path.join(
            model_dir, 'state_{version_str}_{ENCODER.value}.json'.format(**globals()))
        ...
        try:
            self.encoder.load_weights(
                os.path.join(model_dir, hdf['encoderH5']))
    ...
    def Encoder_original(self, **kwargs):
        ...
        in_conv_filters = self.IMAGE_SHAPE[0] if self.IMAGE_SHAPE[0] <= 128 else 128 + (
            self.IMAGE_SHAPE[0]-128)//4
        ...
        x = Dense(self.ENCODER_DIM, kernel_initializer=_kern_init)(
            Flatten()(x))
        x = Dense(dense_shape * dense_shape * 512,
                  kernel_initializer=_kern_init)(x)
    ...
    def Encoder_shaoanlu(self, **kwargs):
        ...
        x = Conv2D(in_conv_filters, kernel_size=5,
                   use_bias=False, padding="same")(impt)
        ...
        x = Dense(self.ENCODER_DIM, kernel_initializer=_kern_init)(
            Flatten()(x))
        x = Dense(dense_shape * dense_shape * 768,
                  kernel_initializer=_kern_init)(x)
    ...
    def save_weights(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(getattr(self, mdl_name.rstrip('H5')).save_weights, str(
                self.model_dir / mdl_H5_fn)) for mdl_name, mdl_H5_fn in hdf.items()]
    ...
    def model_name(self):
        try:
            ...
            import inspect
            self._model_name = os.path.dirname(
                inspect.getmodule(self).__file__).rsplit("_", 1)[1]
        return self._model_name
